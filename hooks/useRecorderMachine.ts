...
export function useRecorderMachine({ user }: UseRecorderOptions) {
  ...
  const emitChunk = useCallback(
    async (blob: Blob, sessionId: string, mimeType: string) => {
      // Use passed sessionId to avoid ref timing issues
      if (!sessionId) {
        console.warn("No session ID provided to emitChunk");
        return;
      }
  ...
  const createRecorder = useCallback(async (source: RecordingSource, sessionId: string) => {
    ...
    try {
      if (source === "TAB") {
        // Check for ScribeAI extension first
        const extension = (window as any).scribeAIExtension;

        if (extension) {
          console.log('Using ScribeAI Extension for tab capture...');
          const response = await extension.startTabCapture();

          if (!response.success || !response.streamId) {
            throw new Error(response.error || 'Failed to get stream ID from extension');
          }

          // Use the stream ID from the extension
          ...
          if (!navigator.mediaDevices.getDisplayMedia) {
            throw new Error('Screen/tab capture is not supported in this browser. Please install the ScribeAI extension.');
          }

          console.log('Requesting tab audio capture via getDisplayMedia...');
    ...
    recorder.ondataavailable = async (event) => {
      ...
      if (event.data && event.data.size > 0) {
        ...
        // Emit chunk to server with explicit session ID and mimeType
        try {
          await emitChunk(event.data, sessionId, recorder.mimeType);
        } catch (error) {
          console.error('Error emitting chunk:', error);
        }
      }
  ...
  const startRecording = useCallback(
    async (source: RecordingSource) => {
      ...
      try {
        ...
        const recorder = await createRecorder(source, sessionId);
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      // Stop extension capture if applicable
      if (state.context.source === 'TAB' && (window as any).scribeAIExtension) {
        (window as any).scribeAIExtension.stopTabCapture();
      }
