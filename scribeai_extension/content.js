  ...
  if (event.data.type === 'SCRIBEAI_START_TAB_CAPTURE') {
    ...
    try {
      if (typeof chrome === 'undefined' || !chrome.runtime) {
        throw new Error('ScribeAI Extension updated. Please reload the extension in chrome://extensions AND refresh this page.');
      }
      ...
      const response = await new Promise((resolve, reject) => {
        try {
          ...
          reject(new Error('Failed to send message to extension. Please refresh the page.'));
        }
      });
